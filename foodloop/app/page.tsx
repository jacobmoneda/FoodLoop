  ...
  const fetchRestaurants = async (lat: number, lng: number) => {
    ...
    try {
      ...
      const popularResponse = await fetch(
        `/api/places?location=-36.8509,174.7645&radius=10000&type=restaurant`,
      );
      const popularData = await popularResponse.json();

      // Fetch nearby restaurants based on user's location
      const nearbyResponse = await fetch(
        `/api/places?location=${lat},${lng}&radius=5000&type=restaurant`,
      );
      ...
      if (popularData.results) {
        ...
        setPopularRestaurants(
          sortedPopular.map((r: any) => ({
            id: r.place_id,
            name: r.name,
            image: r.photos
              ? `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${r.photos[0].photo_reference}&key=${process.env.NEXT_PUBLIC_GOOGLE_PLACES_API_KEY}`
              : "/next.svg",
            rating: r.rating || 0,
            types: r.types,
            vicinity: r.vicinity,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black">
      ...
      <section className="py-3 px-4 dark:border-gray-700">
        <div className="flex flex-col sm:flex-row items-end sm:items-center justify-end gap-4">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex-shrink-0 text-right">
            Welcome, {firstName}
          </h1>
